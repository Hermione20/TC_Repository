...
for(int k=0;k<4;k++)
{
	...
	Chassis_angle.handle_speed[k] = sqrt(Chassis_angle.get_speedw*Chassis_angle.get_speedw + Chassis_angle.Remote_speed*Chassis_angle.Remote_speed \
																				- 2*Chassis_angle.get_speedw*Chassis_angle.Remote_speed*(cosf(Chassis_angle.include_angle[k])));
	...
	Chassis_angle.handle_speed_lim[k] = sqrt(Chassis_angle.get_speedw*Chassis_angle.get_speedw + 
																				...
																				- 2*Chassis_angle.get_speedw*(Chassis_angle.Remote_speed*power_limit_start_flag)*(cosf(Chassis_angle.include_angle[k])));
#else
...
for(int k=0;k<4;k++)
{
	transition1[k] = sinf(Chassis_angle.include_angle[k]);
	...
	Chassis_angle.deviation_angle[k] = asinf(transition3[k]);
...
//	set_3508current_6020voltage();
	...
	{
		vTaskDelay(2000);
		Set_GM6020_IQ1(&hcan1,2000,0,0,0);
	}
}
