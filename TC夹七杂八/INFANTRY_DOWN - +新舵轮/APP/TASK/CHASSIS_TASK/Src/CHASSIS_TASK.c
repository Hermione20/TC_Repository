...
void rotate_follow_gimbal_handle(void)
{
  ...
  chassis.vx = chassis.foward_back_to_foward_back_rotate_speed + chassis.left_right_to_foward_back_rotate_speed;
	chassis .vw =chassis.rotate_speed;
//出事的话加负号
}
#elif CHASSIS_TYPE == 4
void rotate_follow_gimbal_handle(void)
...
void steering_wheel_calc(double Length,double Weight)
{
		...
		{//transition1是各轮同圆半径，transition2是各轮偏航角 
		 //0左前轮 1左后轮 2右后轮 3右前轮 日
			...
			transition1[0]=sqrt((Length/2+R*sinf(theta))*(Length/2+R*sinf(theta))+(Weight/2+R*cosf(theta))*(Weight/2+R*cosf(theta)));
			transition1[1]=sqrt((Length/2-R*sinf(theta))*(Length/2-R*sinf(theta))+(Weight/2+R*cosf(theta))*(Weight/2+R*cosf(theta)));
			transition1[2]=sqrt((Length/2-R*sinf(theta))*(Length/2-R*sinf(theta))+(Weight/2-R*cosf(theta))*(Weight/2-R*cosf(theta)));
			transition1[3]=sqrt((Length/2+R*sinf(theta))*(Length/2+R*sinf(theta))+(Weight/2-R*cosf(theta))*(Weight/2-R*cosf(theta)));
		
			transition2[0]=(  PI/2-atan2((R*cosf(theta)+Length/2),( R*sinf(theta)+Weight/2)));
			transition2[1]=(-(PI/2-atan2((R*cosf(theta)+Length/2),(-R*sinf(theta)+Weight/2))));
			transition2[2]=(-(PI/2-atan2((R*cosf(theta)-Length/2),(-R*sinf(theta)+Weight/2))));
			transition2[3]=(  PI/2-atan2((R*cosf(theta)-Length/2),( R*sinf(theta)+Weight/2)));
		 ...
		 if(Chassis_angle.get_speedw<0)
		{
			...
			if(Chassis_angle.Remote_angle>0&&Chassis_angle.Remote_angle<PI/2)
			{		
			retransition1[0]=transition1[1];
			retransition1[1]=transition1[2];
			retransition1[2]=transition1[3];
		...
		{
			retransition2[0]=-(atan2(chassis.vy,chassis.vx)-PI/2);
			retransition2[1]=-(atan2(chassis.vy,chassis.vx)-PI/2);
			retransition2[2]=-(atan2(chassis.vy,chassis.vx)-PI/2);
			retransition2[3]=-(atan2(chassis.vy,chassis.vx)-PI/2);
		...
		for(int i=0;i<4;i++)
		{
		  transition2[i]=transition2[i]*RAD_TO_ANGLE;
		}
...
{	
	for (int i = 0; i < 4; i++)
		{ 
		chassis.cha_pid_3508.speed_fdb[i]=steering_wheel_chassis.Driving_Encoder[i].filter_rate;
		chassis.cha_pid_6020.speed_fdb[i]=steering_wheel_chassis.Heading_Encoder[i].filter_rate;		
		chassis.cha_pid_6020.angle_fdb[i]=convert_ecd_angle_to_0_2pi(steering_wheel_chassis.Heading_Encoder[i].ecd_angle,chassis.cha_pid_6020.angle_fdb[i]);
		if(chassis.cha_pid_6020.angle_fdb[i]>=2*PI)
		  {chassis.cha_pid_6020.angle_fdb[i]-=2*PI;}
	...
	{
	  for(int i=0;i<4;i++)
		chassis.cha_pid_3508.speed_ref[i]=chassis.vx*1.0f;
	}
	else
		for(int i=0;i<4;i++)
		chassis.cha_pid_3508.speed_ref[i]=transition1[i]*fabs(Chassis_angle.get_speedw)*1.0f;
