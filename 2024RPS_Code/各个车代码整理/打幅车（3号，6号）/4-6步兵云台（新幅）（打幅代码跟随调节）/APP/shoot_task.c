...
static void shoot_bullet_handle(void)    
{
  ...
  if((GetShootState() == SHOOTING) && (lock_rotor == 0)&&shot.ctrl_mode==1)          //没有卡弹
  ...
  else                    //清除卡弹
    {
	 ...
	 {
		 small_delay_time++;
		 small_buff_err=pid_pit_small_buff.get-pid_pit_small_buff.set;
		 if(small_buff_err<=0.5&&small_buff_err>=-0.5&&small_delay_time>buff_shoot_flag_time_flag)
		 {
			 BUFF_shoot_flag=1;
			 press_l_flag=1;
		 }
